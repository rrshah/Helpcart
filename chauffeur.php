	...
	<div class="jumbotron">
		<div class="container">
			<div class="row">
			    <div class="col-md-4">
					<br>
					<img src="chauffeur.jpg" title="Chauffeur" class="img-circle" alt="Cook" width="200" height="200"  class="thumbnail img-responsive">
			    </div>

			    <div class="col-md-8">
				<br><br><br><br>
				<p style="text-align: justify">Imagine the bliss of being chauffeured around! Sit back, relax and watch that movie you've been longing for on your tablet, while our trained chauffeur takes you to your destination safely.</p>
			    </div>
			</div>
		</div>
...
<div class="container" style="background-color:white; padding:10px 10px">
	  <form class="form-horizontal" role="form" method="post" action="">
	    <div class="form-group">
	      ...
	      <div class="col-sm-10">          
		<input type="date" name="chauffeur_date" required>
	      </div>
	    </div>
		...
		<button type="submit" class="btn btn-primary" name="submit_chauffeur">Submit</button>
	      </div>
	    </div>
	  </form>
	</div>
	...
	<?php
		...
		if (isset($_POST['submit_chauffeur'])) {
		  chauffeur();
		}
		...
		function chauffeur(){
			try{
				 ...
				 $time = $_POST["chauffeur_time"];
				 $day = $_POST["chauffeur_date"];
				 $is_perm = $_POST["chauffeur_op"];
	...
	echo "$is_perm\n";
				...
				$stmt = $conn->prepare("SELECT DISTINCT worker.worker_id FROM worker LEFT JOIN transaction ON worker.worker_id = transaction.worker_id WHERE transaction.worker_id IS NULL and worker.cat_id = 2");
				$stmt->execute();    
				$result = $stmt -> fetch();
