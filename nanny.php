...
<div>
	...
	<div class="jumbotron">
		<div class="container">
			<div class="row">
			    <div class="col-md-4">
					<br>
					<img src="nanny.jpg" title="Nanny" class="img-circle" alt="Cook" width="200" height="200"  class="thumbnail img-responsive">
			    </div>

			    <div class="col-md-8">
				<br><br><br><br>
				<p style="text-align: justify">Do you ever wish that more help was available with children , so you can sit back and have a little "me time"? Our  Nanny is here to provide an extra pair of hands – whether it’s helping to get the children settled for bed, keeping them entertained or simply bathing or feeding them.</p>
			    </div>
			</div>
		</div>
	...
	<div class="container" style="background-color:white; padding:10px 10px">
	  <form class="form-horizontal" role="form" method="post" action="">
	    <div class="form-group">
	      ...
	      <div class="col-sm-10">          
		<input type="date" name="nanny_date" required>
	      </div>
	    </div>
		...
		<button type="submit" class="btn btn-primary" name="submit_nanny">Submit</button>
	      </div>
	    </div>
	  </form>
	</div>
	...
	<?php
		...
		function caretaker(){
			try{
				 ...
				 $time = $_POST["nanny_time"];
				 $day = $_POST["nanny_date"];
				 $is_perm = $_POST["nanny_op"];
	...
	echo "$is_perm\n";
				 ...
				 $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
					...
					$stmt = $conn->prepare("SELECT DISTINCT worker.worker_id FROM worker LEFT JOIN transaction ON worker.worker_id = transaction.worker_id WHERE transaction.worker_id IS NULL and worker.cat_id = 4");
				$stmt->execute();    
				$result = $stmt -> fetch();
				...
				if ($rows >= 1){
					...
					$mail->Body    = 'Worker Name: '.$worker_name.' Worker Phone: ' .$worker_phone. ' Thank you!';
					$mail->AltBody = 'Worker Name: '.$worker_name.' Worker Phone: ' .$worker_phone. ' Thank you!';
					// $mail->AddAttachment("worker.jpg");  
