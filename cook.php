	...
	<div class="jumbotron">
		<div class="container">
			<div class="row">
			    <div class="col-md-4">
					<br>
					<img src="cook.jpg" title="Cook" class="img-circle" alt="Cook" width="200" height="200"  class="thumbnail img-responsive">
			    </div>

			    <div class="col-md-8">
				<br><br><br><br>
				<p style="text-align: justify">Nothing beats the taste of a healthy home-cooked meal. Our cook service is your answer to providing you with great tasting, healthy, affordable meals in the comfort of your home.</p>
			    </div>
			</div>
		</div>
	...
	<div class="container" style="background-color:white; padding:10px 10px">
	  <form class="form-horizontal" role="form" method="post" action="">
	    <div class="form-group">
	      ...
	      <div class="col-sm-10">          
		<input type="date" name="cook_date" required>
	      </div>
	    </div>
		...
		<button type="submit" class="btn btn-primary" name="submit_cook">Submit</button>
	      </div>
	    </div>
	  </form>
	</div>
...
<?php
	...
	function cook(){
		try{
			 ...
			 $time = $_POST["cook_time"];
			 $day = $_POST["cook_date"];
			 $is_perm = $_POST["cook_op"];
...
echo "$is_perm\n";
			 ...
			 $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				...
				$stmt = $conn->prepare("SELECT DISTINCT worker.worker_id FROM worker LEFT JOIN transaction ON worker.worker_id = transaction.worker_id WHERE transaction.worker_id IS NULL and worker.cat_id = 1");
				$stmt->execute();    
				$result = $stmt -> fetch();
