	...
	<div class="jumbotron">
		<div class="container">
			<div class="row">
			    <div class="col-md-4">
					<br>
					<img src="cleaner.jpg" title="Cleaner" class="img-circle" alt="Cook" width="200" height="200"  class="thumbnail img-responsive">
			    </div>

			    <div class="col-md-8">
				<br><br><br><br>
				<p style="text-align: justify">Your loved one deserves a clean, safe and healthy home. Are you tired, sick of unreliable, maids and servants who don’t turn up for work when you want them around the most? </p>
			    </div>
			</div>
		</div>
...
<div class="container" style="background-color:white; padding:10px 10px">
	  <form class="form-horizontal" role="form" method="post" action="">
	    <div class="form-group">
	      ...
	      <div class="col-sm-10">          
		<input type="date" name="cleaner_date" required>
	      </div>
	    </div>
		...
		<button type="submit" class="btn btn-primary" name="submit_cleaner">Submit</button>
	      </div>
	    </div>
	  </form>
	</div>
	...
	<?php
		...
		function cleaner(){
			try{
				 ...
				 $time = $_POST["cleaner_time"];
				 $day = $_POST["cleaner_date"];
				 $is_perm = $_POST["cleaner_op"];
	...
	echo "$is_perm\n";
				...
				$stmt = $conn->prepare("SELECT DISTINCT worker.worker_id FROM worker LEFT JOIN transaction ON worker.worker_id = transaction.worker_id WHERE transaction.worker_id IS NULL and worker.cat_id = 5");
				$stmt->execute();    
				$result = $stmt -> fetch();
